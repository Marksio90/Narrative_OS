...
export default function NarrativeDesktop() {
  ...
  const quickActions = [
    { label: t('newChapter'), icon: FileText, action: () => router.push('/chapters/new'), shortcut: 'Ctrl+N' },
    { label: tNav('storyBible'), icon: Book, action: () => router.push('/story-bible'), shortcut: 'Ctrl+B' },
    { label: t('aiAssistant'), icon: Sparkles, action: () => {}, shortcut: 'Ctrl+K' },
    { label: t('statistics'), icon: BarChart3, action: () => router.push('/analytics'), shortcut: 'Ctrl+A' },
    { label: tNav('projects'), icon: Folder, action: () => router.push('/projects'), shortcut: 'Ctrl+P' },
  ]
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                <span className="text-white font-medium">{currentProject?.name || t('selectProject')}</span>
              </button>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              >
                ...
                <span className="text-sm text-gray-300">{t('quickActions')}</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">
            {t('welcome', { username: session?.user?.email?.split('@')[0] || 'Pisarzu' })}
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl p-6 shadow-xl">
            ...
            <p className="text-sm text-white opacity-80 mb-1">{t('todayWritten')}</p>
            <p className="text-3xl font-bold text-white">{stats.today_words.toLocaleString()}</p>
            <p className="text-xs text-white opacity-70 mt-2">{t('words')}</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-orange-500 to-red-600 rounded-xl p-6 shadow-xl">
            ...
            <p className="text-sm text-white opacity-80 mb-1">{t('streak')}</p>
            <p className="text-3xl font-bold text-white">{stats.streak_days}</p>
            <p className="text-xs text-white opacity-70 mt-2">{t('streakDays')}</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl p-6 shadow-xl">
            ...
            <p className="text-sm text-white opacity-80 mb-1">{t('projectProgress')}</p>
            ...
            <p className="text-xs text-white opacity-70 mt-2">
              {currentProject?.current_words.toLocaleString()} / {currentProject?.target_words.toLocaleString()} {t('words')}
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-purple-500 to-pink-600 rounded-xl p-6 shadow-xl">
            ...
            <p className="text-sm text-white opacity-80 mb-1">{t('completed')}</p>
            <p className="text-3xl font-bold text-white">{stats.chapters_completed}</p>
            <p className="text-xs text-white opacity-70 mt-2">{t('chapters')}</p>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 bg-white bg-opacity-10 backdrop-blur-lg rounded-xl p-6 border border-white border-opacity-10">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <p className="text-white font-semibold">{t('newChapter')}</p>
                <p className="text-xs text-purple-200 mt-1">{t('startWriting')}</p>
              </button>
              <button
              ...
              >
                ...
                <p className="text-white font-semibold">{tNav('storyBible')}</p>
                <p className="text-xs text-emerald-200 mt-1">{t('manageCanon')}</p>
              </button>
              <button className="p-4 bg-gradient-to-br from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 rounded-lg transition">
                ...
                <p className="text-white font-semibold">{t('aiAssistant')}</p>
                <p className="text-xs text-pink-200 mt-1">{t('generateAnalyze')}</p>
              </button>
              <button
              ...
              >
                ...
                <p className="text-white font-semibold">{t('statistics')}</p>
                <p className="text-xs text-amber-200 mt-1">{t('analyzeProgress')}</p>
              </button>
            </div>
          </div>
              ...
              return (
                <div key={day} className="flex-1 flex flex-col items-center">
                  <div className="w-full bg-gradient-to-t from-purple-600 to-indigo-500 rounded-t-lg transition-all hover:from-purple-500 hover:to-indigo-400 cursor-pointer"
                    style={{ height: `${height}%` }}
                  ></div>
                  <p className="text-xs text-gray-400 mt-2">{t(`days.${day}`)}</p>
                </div>
              )
            })}
          </div>
